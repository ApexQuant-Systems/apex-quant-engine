...
class PortfolioScheduler(UnifiedPortfolioValidator):
    """
    Layer 9.4: Institutional Portfolio Scheduler & Signal Prioritization Engine.
    Intercepts concurrent multi-cartridge alerts, scores entry quality dynamically,
    and enforces strict global portfolio heat limits to manage simultaneous risk.
    """
    ...
    def execute_scheduled_simulation(self, global_ledger, target_years):
        ...
        df_ledger = df_ledger[df_ledger['year'].isin(target_years)].sort_values('entry_timestamp')
        unique_entry_times = df_ledger['entry_timestamp'].unique()

        for current_time in unique_entry_times:
            still_active = []
            for pos in active_positions:
                if current_time >= pos['exit_timestamp']:
            ...
            concurrent_signals = df_ledger[df_ledger['entry_timestamp'] == current_time].to_dict('records')
        ...
        return total_trades, net_pf, max_dd * 100, balance, recovery_factor

    def run_scheduler_audit(self):
        print("==========================================================================================")
        print(" 🧠 APEX QUANT OS CORE: CENTRAL PORTFOLIO SCHEDULER & CAPITAL RISK ALLOCATION")
        print("==========================================================================================")
        ...
        print("\n[EVALUATING SCHEDULER PERFORMANCE OVER UN-BLINDED 2026 OUT-OF-SAMPLE FRONTIER]")
        print("------------------------------------------------------------------------------------------")
        
        control_weights = {"BTCUSDT": 0.33, "ETHUSDT": 0.33, "SOLUSDT": 0.33}
        
        # FIXED: Mapped all 6 returned parameters from the base simulation pass cleanly
        c_tr, c_wr, c_pf, c_dd, c_bal, c_rf = self.execute_timeline_simulation(global_ledger, control_weights, self.initial_balance, [2026])
        
        s_tr, s_pf, s_dd, s_bal, s_rf = self.execute_scheduled_simulation(global_ledger, [2026])
        
        print(f"▶️ CONTROL PATH (UN-SCHEDULED BLIND ALLOCATION):")
        print(f"  ├── Executed Trades  : {c_tr} Trades")
        print(f"  └── Profit Factor    : {c_pf:.2f} | Max Drawdown: {c_dd:.1f}% | Recovery Factor: {c_rf:.2f}")
        print(f"")
        print(f"▶️🔒 SCHEDULER PATH (PRIORITIZED PORTFOLIO BRAIN):")
        print(f"  ├── Scheduled Trades : {s_tr} Trades")
        print(f"  └── Profit Factor    : {s_pf:.2f} | Max Drawdown: {s_dd:.1f}% | Recovery Factor: {s_rf:.2f}")
        
        status = "🟢 [SUCCESS] Scheduler expanded portfolio recovery efficiency." if s_rf > c_rf else "🔴 [REJECT] Scheduling underperformed."
        print(f"\nSYSTEM VERDICT: {status}")
        print("==========================================================================================")

if __name__ == "__main__":
    scheduler = PortfolioScheduler()
    scheduler.run_scheduler_audit()
