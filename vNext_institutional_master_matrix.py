...
@dataclass
class InstitutionalPerformance:
    asset: str
    set_name: str
    htf_tf: str
    mtf_tf: str
...
class FastHorizonEngine:
    """
    Lookahead-free precomputation of structural states & keyzones across a timeframe series.
    """
...
class InstitutionalMasterMatrixBacktester:
    def __init__(self, 
                 initial_capital_per_cartridge: float = 10000.0,
                 taker_fee_pct: float = 0.0005,  # 0.05% Binance VIP / Standard taker fee
                 base_slippage_pct: float = 0.0002, # 2 bps execution slippage
                 enable_mc_sim: bool = True):
        self.initial_capital = initial_capital_per_cartridge
        self.taker_fee_pct = taker_fee_pct
        self.base_slippage_pct = base_slippage_pct
        self.enable_mc_sim = enable_mc_sim
    ...
    def load_and_downsample_asset(self, asset: str):
        ...
        print(f"  └─ Ingested {len(df):,} 1m bars in {time.time()-t0:.2f}s.")
        ...
        # Precompute FastHorizonEngines, ADX, and ATR
        print(f"  └─ Vectorizing structural pivots, ADX, and ATR volatility models...")
        t2 = time.time()
    ...
    def _monte_carlo_stress_test(self, pnl_list: List[float], initial_capital: float, iterations: int = 1000) -> float:
        """
        Runs Monte Carlo trade sequence permutations to derive 95% worst-case Max Drawdown.
        """
        if len(pnl_list) < 5:
            return 0.0
        ...
        for _ in range(iterations):
            shuffled_pnl = np.random.permutation(pnl_arr)
    ...
    def run_single_simulation(self, asset: str, set_name: str, config: dict, ds: Dict[str, pd.DataFrame]) -> InstitutionalPerformance:
        htf_tf = config['htf']
        mtf_tf = config['mtf']
        ltf_tf = config['ltf']
        ...
        for i in range(warmup, len(df_ltf)):
            ...
            # Dynamic slippage factor = base slippage + 5% of ATR relative spread
            slip_pct = self.base_slippage_pct + (0.05 * (current_atr / current_close))
            ...
            if in_trade:
                ...
                if not has_taken_partial:
                    if side == 'LONG' and current_high >= partial_tp_price:
                        ...
                        slip_loss = (partial_tp_price - realized_exit) * (position_size * 0.5)
                        fee_exit = realized_exit * (position_size * 0.5) * self.taker_fee_pct
                        
                        gross_p = (realized_exit - realized_entry_price) * (position_size * 0.5)
                        net_p = gross_p - fee_exit
                        
                        capital += net_p
                        trade_pnls.append(net_p)
                        total_fee_drag += fee_exit
                        total_slippage_drag += slip_loss
                        ...
                        realized_exit = partial_tp_price * (1.0 + slip_pct)
                        slip_loss = (realized_exit - partial_tp_price) * (position_size * 0.5)
                        fee_exit = realized_exit * (position_size * 0.5) * self.taker_fee_pct
                        
                        gross_p = (realized_entry_price - realized_exit) * (position_size * 0.5)
                        net_p = gross_p - fee_exit
                ...
                if side == 'LONG':
                    if current_low <= sl_price:
                        ...
                        slip_loss = (sl_price - realized_exit) * position_size
                        fee_exit = realized_exit * position_size * self.taker_fee_pct
                        
                        gross_p = (realized_exit - realized_entry_price) * position_size
                        net_p = gross_p - fee_exit
                        ...
                        slip_loss = (tp_price - realized_exit) * position_size
                        fee_exit = realized_exit * position_size * self.taker_fee_pct
                    ...
                    if current_high >= sl_price:
                        realized_exit = sl_price * (1.0 + slip_pct)
                        slip_loss = (realized_exit - sl_price) * position_size
                        fee_exit = realized_exit * position_size * self.taker_fee_pct
                        
                        gross_p = (realized_entry_price - realized_exit) * position_size
                        net_p = gross_p - fee_exit
                        ...
                        realized_exit = tp_price * (1.0 + slip_pct)
                        slip_loss = (realized_exit - tp_price) * position_size
                        fee_exit = realized_exit * position_size * self.taker_fee_pct
            ...
            # -------------------------------------------------------------
            # Strategy Finite State Machine (Lookahead-Free Step)
            # -------------------------------------------------------------
                        ...
                        if sl != 0.0 and tp != 0.0:
                            ...
                            if risk_guardian.evaluate_intent(intent, current_equity=capital):
                                ...
                                if intent.side.name == 'LONG':
                                    realized_entry_price = entry_price * (1.0 + slip_pct)
                                else:
                                    realized_entry_price = entry_price * (1.0 - slip_pct)
                                ...
                                if risk_dist > 0:
                                    ...
                                    fee_entry = realized_entry_price * position_size * self.taker_fee_pct
                                    slip_entry = abs(realized_entry_price - entry_price) * position_size
                                    
                                    capital -= fee_entry
                                    total_fee_drag += fee_entry
                                    total_slippage_drag += slip_entry
        ...
        # Calculate institutional metrics (Sharpe, Sortino, CAGR, Calmar)
        ...
        mc_95_dd = self._monte_carlo_stress_test(trade_pnls, self.initial_capital, iterations=500) if self.enable_mc_sim else max_drawdown
        
        start_str = str(df_ltf.iloc[0]['datetime'])[:10]
        end_str = str(df_ltf.iloc[-1]['datetime'])[:10]
        
        return InstitutionalPerformance(
            asset=asset,
            set_name=set_name,
            htf_tf=htf_tf,
            mtf_tf=mtf_tf,
    ...
    def run_master_matrix(self):
        print("=" * 108)
        print("🏛️  APEX QUANT OS: INSTITUTIONAL MASTER MATRIX PERFORMANCE VERIFICATION")
        print("   Friction Injected: 0.05% Taker Commission + ATR Dynamic Slippage + Monte Carlo Stress Testing")
        print("=" * 108)
        
        master_t0 = time.time()
        results: List[InstitutionalPerformance] = []
        
        for asset in self.assets.keys():
            ds = self.load_and_downsample_asset(asset)
            
            print(f"\n[⚡ SIMULATING REAL-WORLD EXECUTION] Asset: {asset}")
            print("─" * 108)
            for set_name, config in self.timeframe_sets.items():
                print(f"  ▶ Cartridge: [{set_name:<25}] (HTF:{config['htf']:<3} -> MTF:{config['mtf']:<3} -> LTF:{config['ltf']:<5})...", end="", flush=True)
                t_sub = time.time()
                perf = self.run_single_simulation(asset, set_name, config, ds)
                if perf:
                    results.append(perf)
                    print(f" Done ({time.time()-t_sub:4.2f}s) | Trades: {perf.total_trades:3d} | WR: {perf.win_rate_non_loss:4.1f}% | Net ROI: {perf.net_return_pct:+6.1f}% | Sharpe: {perf.sharpe_ratio:4.2f} | MC 95% DD: {perf.mc_95_max_dd:4.1f}%")
                    
        # Summary Matrix Display
        print("\n" + "=" * 122)
        print("                               📊 INSTITUTIONAL RISK & ALPHA MATRIX (NET OF ALL FRICTION)")
        print("=" * 122)
        ...
        print("\n" + "=" * 90)
        print("🏆 CONSOLIDATED INSTITUTIONAL PORTFOLIO TEAR SHEET")
        print("=" * 90)
        ...
        print(f"Total Engine Execution Time:          {time.time() - master_t0:.2f} seconds")
        print("=" * 90)

if __name__ == "__main__":
    backtester = InstitutionalMasterMatrixBacktester(
        initial_capital_per_cartridge=10000.0,
        taker_fee_pct=0.0005,      # 0.05%
        base_slippage_pct=0.0002,  # 2 bps
        enable_mc_sim=True
    )
